...
pwm_serv_init(GPIO_TypeDef *GPIOx, uint32_t Pin, TIM_TypeDef *TIMx, uint32_t Periphs, uint32_t Channel) {
	...
	const uint32_t TIM_CLOCK_FREQ = 100000;
...
static void
pwm_serv_set(LL_TIM_OC_InitTypeDef motor_OC_s, TIM_TypeDef *TIMx, uint32_t Channel, int Percentage) {
	motor_OC_s.CompareValue = ( (LL_TIM_GetAutoReload(TIM3) + 1) / 1000 * (Percentage + 26));
	//motor_OC_s.CompareValue = ( (LL_TIM_GetAutoReload(TIM3) + 1) / 10);
